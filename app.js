...
const getStates = async () => {
    const res = await fetch('https://gist.githubusercontent.com/P1N2O/5778a23dbd87d4dbb946523ca6e951a9/raw/4e5120897df78094b827bc7e3a87ac4f9175225a/data.json');
...
const searchStates = searchText => {
    ...
    let matches = states.filter(state => {
        ...
        return state.partnumber.match(regex);
    });
...
const outputHtml = matches => {
    ...
    <h4>${match.partnumber}<span class="quantity float-right">[${match.quantity}]</span></h4>
    <h6>Condition: ${match.condition}</h6>
    <small>${match.description}</small>
        ...
        // Change color if Qty is 0
        $("span.quantity").each(function () {
            $(this).html() == '[0]' ? $(this).css('color', '#dc3545') : null;
            $(this).html() != '[0]' ? $(this).css('color', '#007bff') : null;
        });
